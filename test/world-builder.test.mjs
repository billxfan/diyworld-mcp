...
test("the platform seeds one World Builder Agent, host templates, and official provenance", () => {
  ...
  try {
    ...
    assert.ok(
      result.templates.some((template) => template.id === "persistent-sandbox"),
    );
    assert.equal(
    ...
    const officialBuilds = db
      ...
      `)
      .all();
    assert.equal(officialBuilds.length, 20);
    for (const build of officialBuilds) {
      ...
      assert.equal(build.platform_agent_policy_version, 1);
...
test("the World Builder infers a Host family and reports five-view readiness", () => {
  ...
  try {
    ...
    assert.equal(started.template_id, "story-host");
    assert.equal(
